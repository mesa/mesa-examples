...
def plot_results(model, model_df, save_path="simulation_results.png"):
    ...
    ax = axes[0]
    ax.step(range(1, len(demand) + 1), demand, where="post", label="Demand", color="tab:blue")
    ax.step(range(1, len(supply) + 1), supply, where="post", label="Supply", color="tab:red")
    if eq_price is not None:
        ax.axhline(eq_price, color="gray", linestyle="--", linewidth=1,
    ...
    if model.price_history:
        ...
        ax.plot(times, prices, marker="o", markersize=3, linewidth=1,
                color="tab:green", label="Transaction price")
    if eq_price is not None:
        ax.axhline(eq_price, color="gray", linestyle="--", linewidth=1,
    ...
    ax = axes[2]
    ax.plot(model_df.index, model_df["CumulativeVolume"], color="tab:purple", label="Cumulative volume")
    ax2 = ax.twinx()
    ax2.plot(model_df.index, model_df["Spread"], color="tab:orange", linewidth=1, label="Spread")
    ...
    if eq_price is not None and model.price_history:
        last_5 = sum(p for _, p in model.price_history[-5:]) / min(5, len(model.price_history))
...
if __name__ == "__main__":
    model, model_df, agent_df = run(
        n_buyers=25, n_sellers=25, max_valuation=100.0,
        mean_interarrival=1.0, duration=300, rng=42,
    )
    print(model_df.tail(10))
    plot_results(model, model_df, save_path=os.path.join(os.path.dirname(__file__), "simulation_results.png"))
