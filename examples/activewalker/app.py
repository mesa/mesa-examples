...
def agent_portrayal(agent):
    if isinstance(agent, StopsAgent):
        return AgentPortrayalStyle(size=200, marker="house.png", zorder=100)
...
model_params = {
    "population_size": Slider(label="Number", value=80, min=10, max=200, step=10),
    "speed_mean": Slider(label="speed", value=1.0, min=0.1, max=2.0, step=0.1),
    "default_value": Slider(
        label="min trail strength", value=0, min=0, max=1.0, step=0.1
    ),
    "max_value": Slider(label="max trail strength", value=10, min=1, max=50, step=1),
    "print_strength": Slider(
        label="agent print strength", value=5, min=1, max=50, step=5
    ),
    "trail_dies_in": Slider(
        label="trail dies in", value=2000, min=100, max=2001, step=100
    ),
    "vision": Slider(label="vision", value=4, min=2, max=10, step=1),
    "resolution": Slider(
        label="resolution of trail grid", value=2, min=1, max=10, step=1
    ),
    "rng": None,
    "stops": [(5, 5), (35, 5), (20, 35)],
    "width": 40,
