...
class VillagerAgent(NeedsAgent):
    ...
    def _nearest(self, agent_type: type):
        ...
        return min(
            pool,
            key=lambda a: (
                abs(a.cell.coordinate[0] - my_x) + abs(a.cell.coordinate[1] - my_y)
            ),
        )
    ...
    def discretise_needs(self, buckets: int = 3) -> tuple[int, ...]:
        ...
        utility. With 4 needs x 3 buckets = 81 discrete states; including
        position on a 25x25 grid gives 81 x 625 = 50625 states.
        """
        step = 1.0 / buckets
