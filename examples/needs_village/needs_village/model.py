...
class VillageModel(mesa.Model):
    ...
    def __init__(
        ...
        self.datacollector = mesa.DataCollector(
            ...
            agent_reporters={
                "Hunger": lambda a: (
                    a.needs.get("HUNGER") if isinstance(a, VillagerAgent) else None
                ),
                "Rest": lambda a: (
                    a.needs.get("REST") if isinstance(a, VillagerAgent) else None
                ),
                "Social": lambda a: (
                    a.needs.get("SOCIAL") if isinstance(a, VillagerAgent) else None
                ),
                "Safety": lambda a: (
                    a.needs.get("SAFETY") if isinstance(a, VillagerAgent) else None
                ),
                "ActiveNeed": lambda a: (
                    a._active_need if isinstance(a, VillagerAgent) else None
                ),
                "Preemptions": lambda a: (
                    a._preemption_count if isinstance(a, VillagerAgent) else None
                ),
            },
        )
    ...
    def step(self) -> None:
        ...
        self.agents.select(agent_type=ThreatAgent).shuffle_do("step")
        self.agents.select(agent_type=FoodSource).shuffle_do("step")
        self.agents.select(agent_type=VillagerAgent).shuffle_do("step")
