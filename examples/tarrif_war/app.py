...
from tarrif_war.agent import State, Organization, Resident
...
def BetaLobbyingPlot(model):
    ...
    if len(df) >= 2:
        ...
        ax1.plot(df.index, df["Beta_China"],  color=COLORS["China"],        lw=2,   label="β China")
        ax1.plot(df.index, df["Beta_Neutral"],color=COLORS["Neutral Asia"], lw=1.5, linestyle="--", label="β Neutral Asia")
        ...
        ax2.plot(df.index, df["Lobby_USA"],   color=COLORS["USA"],   lw=1.0, alpha=0.6, linestyle="-.")
        ax2.plot(df.index, df["Lobby_China"], color=COLORS["China"], lw=1.0, alpha=0.6, linestyle="-.")
    ...
    ax1.set_title("Firm Pessimism (β) & Lobbying Buildup", fontsize=9, fontweight="bold")
    ax1.set_xlabel("Step")
    fig.tight_layout()
...
def ExpectationProductionPlot(model):
    ...
    if len(df) >= 2:
        ax.plot(df.index, df["Global_Expected_Demand"],  color="#4472C4", lw=2,   label="Expected Demand")
        ax.plot(df.index, df["Global_Actual_Production"], color="#ED7D31", lw=2,   label="Actual Production", linestyle="--")
        ax.fill_between(df.index,
                        df["Global_Expected_Demand"],
                        df["Global_Actual_Production"],
                        alpha=0.12, color="#7030A0", label="Forecast Gap")
...
def GovSizePlot(model):
    ...
    if len(df) >= 2:
        ax.plot(df.index, df["Avg_Gov_Size"], color="#7030A0", lw=2.5, label="Avg Gov Size")
        ax.axhline(y=20.0, color="gray", lw=0.9, linestyle=":", label="Peacetime baseline (20% GDP)")
...
def GDPPlot(model):
    ...
    if len(df) >= 2:
        ...
        ax1.plot(df.index, df["China_GDP"],       color=COLORS["China"],        lw=2, label="China GDP")
        ax1.plot(df.index, df["NeutralAsia_GDP"], color=COLORS["Neutral Asia"], lw=2, label="Neutral Asia GDP")
        ax1.set_ylabel("Total Production (GDP proxy)")

        ax2.fill_between(df.index, df["TradeBalance_USA"],     alpha=0.15, color=COLORS["USA"])
        ax2.fill_between(df.index, df["TradeBalance_Neutral"], alpha=0.15, color=COLORS["Neutral Asia"])
...
def FirmHealthPlot(model):
    ...
    if len(df) >= 2:
        ax1.plot(df.index, df["Bankruptcy_USA"]     * 100, color=COLORS["USA"],          lw=2.5, label="USA bankruptcy %")
        ax1.plot(df.index, df["Bankruptcy_China"]   * 100, color=COLORS["China"],        lw=2.5, label="China bankruptcy %")
        ax1.plot(df.index, df["Bankruptcy_Neutral"] * 100, color=COLORS["Neutral Asia"], lw=2,   linestyle="--", label="Neutral bankruptcy %")
        ax1.set_ylabel("Firm Bankruptcy Rate (%)", fontsize=10)
        ax1.set_ylim(0, 105)

        ax2.plot(df.index, df["AvgProfit_USA"],     color=COLORS["USA"],          lw=1.5, alpha=0.6, linestyle="-.")
        ax2.plot(df.index, df["AvgProfit_China"],   color=COLORS["China"],        lw=1.5, alpha=0.6, linestyle="-.")
        ax2.plot(df.index, df["AvgProfit_Neutral"], color=COLORS["Neutral Asia"], lw=1.5, alpha=0.6, linestyle="-.")
    ...
    ax1.set_title("Firm Health: Bankruptcy Rate & Avg Profit", fontsize=11, fontweight="bold")
...
def WelfarePlot(model):
    ...
    if len(df) >= 2:
        ...
        ax.plot(df.index, df["Welfare_China"],   color=COLORS["China"],        lw=2, label="China")
        ax.plot(df.index, df["Welfare_Neutral"], color=COLORS["Neutral Asia"], lw=2, label="Neutral Asia")
        ax.fill_between(df.index, df["Welfare_USA"],     alpha=0.07, color=COLORS["USA"])
        ax.fill_between(df.index, df["Welfare_Neutral"], alpha=0.07, color=COLORS["Neutral Asia"])
...
model_params = {
    ...
    "lobbying_sensitivity": {
        "type": "SliderFloat",
        "value": 0.25, "min": 0.00, "max": 0.80, "step": 0.05,
        "label": "Lobbying Sensitivity",
    },
    "expectation_adaptation_rate": {
        "type": "SliderFloat",
        "value": 0.15, "min": 0.01, "max": 0.40, "step": 0.01,
        "label": "Expectation Adaptation Rate (β speed)",
    },
    "production_elasticity": {
        "type": "SliderFloat",
        "value": 0.40, "min": 0.10, "max": 0.80, "step": 0.05,
        "label": "Production Elasticity",
    },
    "trade_stickiness": {
        "type": "SliderFloat",
        "value": 0.40, "min": 0.00, "max": 0.90, "step": 0.05,
