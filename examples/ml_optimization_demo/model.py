...
class MLDemoModel(mesa.Model):
    ...
    def optimize_parameters(self, n_iterations=30):
        ...
        for i in range(10):
            ...
            x.append(
                [params["num_agents"], params["ratio"], params["exploration_bias"]]
            )
            y.append(score)
            phase.append(1)
            print(
                f"  {i + 1:2d}. agents={params['num_agents']:2d}, "
        ...
        for i in range(20):
            ...
            if score > best_score:
                ...
                best_params = params
                print(
                    f"  {i + 1:2d}. NEW BEST - agents={params['num_agents']:2d}, "
                    f"ratio={params['ratio']:.2f}, "
                    f"bias={params['exploration_bias']:.2f}, score={score:.4f}"
                )
            else:
        ...
        improvement_pct = (
            (best_score - np.max(random_scores)) / np.max(random_scores) * 100
        )
...
def visualize_optimization(result, save_path=None):
    ...
    ml_idx = phases == 2
    ax.scatter(
        np.where(random_idx)[0],
        all_scores[random_idx],
        color="red",
        s=100,
        alpha=0.6,
        label="Random Search",
    ...
    ax.set_title(
        "Optimization Performance: Random vs Bayesian", fontsize=13, fontweight="bold"
    )
    ...
    ax.plot(
        range(
            len(running_best_random), len(running_best_random) + len(running_best_ml)
        ),
        running_best_ml,
        "s-",
        color="blue",
    ...
    ax.text(
        5, ax.get_ylim()[1] * 0.95, "Phase 1", ha="center", fontsize=10, style="italic"
    )
    ax.text(
        20, ax.get_ylim()[1] * 0.95, "Phase 2", ha="center", fontsize=10, style="italic"
    )
    ax.set_xlabel("Iteration", fontsize=12, fontweight="bold")
