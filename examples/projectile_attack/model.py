...
class RandomActivation:
    ...
    def step(self):
        ...
        for agent in self._agents[
            :
        ]:  # use slice copy to avoid mutation during iteration
...
class TankGameModel(Model):
    ...
    def reset(self):
        ...
        self.target = Target(
            self, target_id, pos_f=(self.grid.width - 2, float(target_y))
        )
    ...
    def step(self):
        ...
        if (
            self.wall_position != self._last_wall_position
            or self.wall_height != self._last_wall_height
        ):
            self._build_wall_cells()
