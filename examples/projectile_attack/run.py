...
from agents import Cloud, Shell, Tank, Target
...
class TankGameVisualization:
    ...
    def _create_controls(self, parent):
        ...
        self.angle_label = ttk.Label(
            angle_frame, text=f"{self.default_params['angle']:.1f}"
        )
        ...
        self.power_label = ttk.Label(
            power_frame, text=f"{self.default_params['power']:.1f}"
        )
        ...
        self.wind_label = ttk.Label(
            wind_frame, text=f"{self.default_params['wind']:.1f}"
        )
        ...
        ttk.Label(wall_pos_frame, text="Wall Position:").grid(
        ...
        self.wall_pos_label = ttk.Label(
            wall_pos_frame, text=str(self.default_params["wall_position"])
        )
        ...
        ttk.Label(wall_height_frame, text="Wall Height:").grid(
            row=0, column=0, sticky=tk.W
        )
        ...
        self.wall_height_label = ttk.Label(
            wall_height_frame, text=str(self.default_params["wall_height"])
        )
        self.wall_height_label.grid(row=0, column=2, sticky=tk.W)

        # Wall blocks wind checkbox
        self.wall_block_wind_var = tk.BooleanVar(
            value=self.default_params["wall_block_wind"]
        )
        ...
        self.target_movable_var = tk.BooleanVar(
            value=self.default_params["target_movable"]
        )
        ...
        # Fire button
        fire_button = ttk.Button(button_frame, text="Fire", command=self._on_fire)
    ...
    def update_display(self):
        ...
        for i in range(width + 1):
            ...
            self.canvas.create_line(
                x, 0, x, height * cell_size, fill="lightgray", width=1
            )
        for i in range(height + 1):
            y = i * cell_size
            self.canvas.create_line(
                0, y, width * cell_size, y, fill="lightgray", width=1
            )
            ...
            for dx, dy in self.cloud_word_cells:
                ...
                if 0 <= y < height:
                    ...
                    self.canvas.create_rectangle(
                        x1, y1, x2, y2, fill="#dcdcdc", outline=""
                    )
        ...
        for x, y in self.model.trajectory_cells:
            ...
            y2 = (height - y) * cell_size
            self.canvas.create_rectangle(
                x1, y1, x2, y2, fill="#ff9e9e", outline="", stipple="gray50"
            )
        ...
        if self.model.explosion_centers:  # only draw when explosion centers exist
            for (x, y), _ttl in self.model.explosion_cells.items():
                x1 = x * cell_size
                y1 = (height - 1 - y) * cell_size
                x2 = (x + 1) * cell_size
                y2 = (height - y) * cell_size
                ...
                self.canvas.create_rectangle(
                    x1, y1, x2, y2, fill=color, outline="darkorange"
                )
        ...
        if not self.model.running and not self.model.target_exists:
            ...
            if not self.model.explosion_cells and not self.auto_reset_scheduled:
                ...
                self.auto_reset_after_id = self.root.after(
                    500, self._auto_reset_after_hit
                )
