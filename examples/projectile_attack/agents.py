...
class Shell(Agent):
    ...
    def __init__(self, model, unique_id=None, pos_f: Tuple[float, float] = None, vx: float = 0.0, vy: float = 0.0):
        """
        Initialize a shell.
    ...
    def step(self):
        ...
        if wall_block_wind:
            ...
            if within_wall_height:
                if grid_x == wall_position:
                    in_wall_shadow = True
                elif wind_acc < 0 and grid_x < wall_position:
                    in_wall_shadow = True
                elif wind_acc > 0 and grid_x > wall_position:
                    in_wall_shadow = True
        ...
        if grid_x < 0 or grid_x >= self.model.grid.width or \
           grid_y < 0 or grid_y >= self.model.grid.height:
            self.alive = False
...
class Target(Agent):
    ...
    def __init__(self, model, unique_id=None, pos_f: Tuple[float, float] = None):
        """
        Initialize a target.
...
class Cloud(Agent):
    ...
    def __init__(self, model, unique_id=None, pos_f: Tuple[float, float] = (17.0, 30.0)):
        """
        Initialize a cloud.
